 ...
 */
void benchmark(int64_t (*func)(int64_t[], uint64_t, uint64_t), Parameter *p,
               Measurement *m)
{
 ...
 * @param struct Measurements* m_cols: Messergebniss für iterative Funktion
 * @param struct Measurements* m_rows: Messergebniss für rekursive Funktion
 */
void drawTable(char *name, Measurement *ergs_cols, Measurement *ergs_rows)
{
...
{
	int64_t matrix[M * N];
	Measurement erg_cols, erg_rows;
	Parameter p = {.matrix = matrix, .n = N, .m = M};
	...
	printf("Benchmark: Zeilen = %d & Spalten = %d => Elemente = %d\n\n", M,
	       N, N * M);
